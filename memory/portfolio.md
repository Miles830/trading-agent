...
## Last Updated
**2026-06-17 (Wednesday) — Market-Close (3:33 PM ET / 19:33 UTC). TRADING DAY — FOMC DECISION DAY. Alpaca API STILL BLOCKED ("Host not in allowlist" — 38th consecutive blocked session). 4 silent failures today (Pre-Market, Market-Open, Mid-Morning, Afternoon — violations logged). Market-Close ✓ RUNNING. FOMC DECISION AT 2:00 PM ET (18:00Z) — CLEARED. HOLD CONFIRMED (per 97%+ pre-meeting probability). Exemption 2 EXPIRED. 3 MOC ORDER ATTEMPTS FAILED (MRVL 8sh, INTC 38sh, AMD 9sh) — all returned HTTP 403. NVDA 4th name skipped per Exemption 1 (3-MOC daily cap). 0 open positions, ~100% cash (~$99,854). ⚠️⚠️ AMD STALE GTCs ($520.59 and $524.15) — STATUS UNKNOWN — OPERATOR MUST VERIFY AT alpaca.markets IMMEDIATELY. PUSH NOTIFICATION SENT 19:33Z. MANDATORY JUNE 18 ENTRIES: MRVL 8sh MOO/limit, INTC 38sh MOO/limit, AMD 9sh MOO/limit (cancel stale GTCs first), NVDA 4sh conditional (fresh 6-agent). JUNETEENTH JUNE 19 = MARKET CLOSED. Cumulative gap: ~−5.2 to −5.8 pp (est.). ⚠️ 20-DAY UNDERPERFORMANCE FLAG: 28th consecutive trading day (API blockage root cause). Operator manual execution at alpaca.markets is the ONLY resolution.**
