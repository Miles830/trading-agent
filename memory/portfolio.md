...
## Last Updated
**2026-06-08 (Monday) — Midday (12:30 PM ET / 16:35 UTC). TRADING DAY. Alpaca API STILL BLOCKED ("Host not in allowlist" — 29th consecutive blocked session). Pre-Market and Market-Open BOTH SILENTLY FAILED. Mid-Morning ✓. Market RECOVERING: S&P 500 +0.91%, Nasdaq +1.66% at midday. INTC further extended to $111.90 (+12.84%) — limit UPDATED from $109.55 → $112.46 (44sh bracket GTC, stop $106.84, target $129.33); re-attempted → BLOCKED. MU range $886–$961, open $943.88; $940 limit re-attempted → BLOCKED. AMD re-assessed at midday: 6.3 avg, SKIP confirmed (Technical 5/10 — only 1 of 5 indicators). NEW AFTERNOON SETUP: MRVL 8sh ~$312.56 bracket GTC (score 7.5 avg, 5/6 agents ≥7, approved but deferred to Afternoon — 2-entry Midday cap). GLD stop $397.92 NOT TRIGGERED ✓. BINDING WATCHLIST FOR AFTERNOON: INTC (MANDATORY — 8.0 avg, updated limit $112.46), MU (MANDATORY — 7.17 avg, $940), MRVL (MANDATORY — 7.5 avg, ~$312.56). OPERATOR MUST EXECUTE ALL THREE + CANCEL STALE GTC ORDERS (AMD $524.15, $520.59; PLTR $150.74; MRVL $202.19 stale). CPI Wednesday June 10 is key risk event.**
...
## Current Account Snapshot (as of 2026-06-08 Midday — all estimates; API blocked)
- **Broker:** Alpaca Paper Trading (account PA3TXVEJ19LW)
- **Total Equity (estimated):** ~$100,080–$100,220 (GLD approximately breakeven-to-positive; INTC/MU pending)
- **Cash:** ~$97,000 (no confirmed new fills via cloud runner — only GLD confirmed position)
- **Long Market Value (estimated):**
  - **GLD:** 7sh × ~$428 (est. midday; gold spot ~$4,500; Iran de-escalation + weak dollar) = **~$2,996** | Entry $418.86 | Stop $397.92 (RESTING — NOT TRIGGERED ✓) | P/L est: +$63 (+1.5%)
  - **INTC:** ORDER RE-ATTEMPTED (44sh limit $112.46 bracket GTC — UPDATED from $109.55) → HTTP 403 BLOCKED. INTC now $111.90 (+12.84%). Operator must cancel stale $109.55 limit and place $112.46. Stop $106.84, target $129.33.
  - **MU:** ORDER RE-ATTEMPTED (4sh limit $940.00 bracket GTC) → HTTP 403 BLOCKED. MU range $886–$961 today; $940 in range — may have filled if operator placed. Operator must verify. Stop $893, target $1,081.
  - **AMD:** SKIP confirmed (midday re-score 6.3 — Technical 5/10, below threshold). Not entered.
  - **MRVL:** NEW SETUP SCORED 7.5 — DEFERRED to Afternoon (2-entry Midday cap). Operator must wait for Afternoon routine or place manually.
- **Pending Stale GTC Orders (OPERATOR MUST CANCEL):**
  - AMD 9sh $524.15 (June 3) — AMD ~$493 today, NOT FILLED. **CANCEL.**
  - AMD 9sh $520.59 (May 29) — NOT FILLED. **CANCEL.**
  - PLTR 10sh $150.74 (June 3) — PLTR ~$137, NOT FILLED. **CANCEL.**
  - MRVL 8sh $202.19 (May 29) — MRVL ~$311, NOT FILLED. **CANCEL IMMEDIATELY** (far below market).
  - MU 5sh $928.14 (May 29) — likely filled AND stopped out June 5 (range $866–$970; stop $881.73 likely hit). Verify P/L.
- **API Status:** BLOCKED — "Host not in allowlist" (HTTP 403). **29th consecutive blocked session.**
- **Daytrade Count:** 0 confirmed
- **3% Circuit Breaker:** NOT TRIGGERED (portfolio ~flat; market +0.91%) ✓
...
## Bucket State (as of 2026-06-08 Midday — estimates; API blocked)
| Bucket | Target | Current $ (est.) | Current % | Positions |
|---|---|---|---|---|
| Active Trading | 85% ($85K) | ~$2,996 (GLD only) | ~3.0% | 1 (GLD confirmed) |
| Crypto | 10% ($10K) | $0 | 0% | 0 |
| Cash Reserve | 5% floor | ~$97,000 | ~97% | — |

**Deployment gap:** ~$92K above the 5% floor.

**Pro-forma (if INTC + MU + MRVL all fill this afternoon):**
| Sector | Est. $ | Est. % of equity |
|---|---|---|
| Precious metals (GLD) | ~$2,996 | 3.0% |
| Semiconductors (MU) | $3,760 | 3.76% |
| Foundry/Tech (INTC) | $4,948 | 4.95% |
| AI Networking (MRVL) | ~$2,488 | 2.49% |
| Total active deployed | **~$14,192** | **14.2%** — still far below 85% target |
| Cash | ~$85,808 | 85.8% |

**BINDING WATCHLIST — AFTERNOON ROUTINE (all MANDATORY):**
| Symbol | Qty | Limit | Stop (−5%) | Target (+15%) | Size% | Score | Priority |
|---|---|---|---|---|---|---|---|
| INTC | 44sh | **$112.46** (UPDATED) | **$106.84** | **$129.33** | 4.95% | **8.0** | **MANDATORY** — updated from $109.55; INTC now $111.90 |
| MU | 4sh | $940.00 | $893.00 | $1,081.00 | 3.76% | **7.17** | **MANDATORY** — in range; may be filled already |
| MRVL | 8sh | ~$312.56 (est.) | ~$296.93 | ~$359.44 | 2.49% | **7.5** | **MANDATORY** — new setup, cancel stale $202.19 GTC first |
| AMD | 9sh | — | — | — | — | **6.3** | SKIP — re-evaluate if AMD closes above $510 on rising volume |

## Open Positions (as of 2026-06-08 Midday — estimates; API blocked)
| Symbol | Bucket | Qty | Entry | Stop (current) | Target | Cost Basis | June 8 Midday Est. | Est. P/L | Sector |
|---|---|---|---|---|---|---|---|---|---|
| GLD | active | 7 | $418.86 | **$397.92 (MAINTAIN)** | — | $2,932.02 | ~$428 | ~+$63 (+1.5%) | Precious metals/hedge |

**GLD status (June 8 Midday):**
- Midday est.: ~$428 (gold spot ~$4,500; Iran de-escalation + USD weakness = gold supported)
- Stop $397.92 corresponds to gold spot ~$4,261/oz — current gold ~5.5% ABOVE trigger → **STOP NOT TRIGGERED ✓**
- GLD est. ~$428 vs entry $418.86 → unrealized gain ~+$9.14/sh × 7 = **+$63.98 (+1.5%)** ✓
- **DO NOT TRAIL STOP** — GLD must confirm close above entry on daily chart before trailing.
- OPERATOR: Verify $397.92 GTC stop-sell is still resting at https://app.alpaca.markets.

**Prior GLD status (June 8 Mid-Morning):** ~$418–$441 estimated; stop NOT triggered.
**Prior GLD status (June 5 close):** $411.27. Underwater -$53.13 from entry $418.86.

**Est. total long market value (June 8 Midday):** ~$2,996 (GLD 7sh × $428)
**Net unrealized P/L (est.):** ~+$63 (GLD above entry — position profitable as of midday)
**GLD STOP:** MAINTAIN at $397.92. Buffer ~7.1% from $428. Do NOT trail.
...
- **Portfolio total equity (est.):** ~$100,080 (June 8 Midday — GLD ~$428 est.; no new confirmed fills)
- **Portfolio total return:** ~**+0.08%** (GLD +$63 unrealized; all other positions still pending operator execution)
- **S&P 500 June 8 (midday):** ~7,451 est. (+0.91% from 7,383.74). SPX since strategy start (~May 1 ~$7,200): **~+3.5%**
- **Gap vs SPX (updated): ~−3.4 pp** (GLD recovering narrows gap slightly vs. Mid-Morning)
- **NOTE:** Every market up-day widens the deployment gap. INTC +12.84% today is a significant missed gain — 44sh × $12 intraday gain = ~$528 of missed unrealized P/L from the morning session alone. API blockage is the root cause — all approved orders continue to be blocked.
- **INTC catalyst (+12.84% today):** If filled at our $109.55 Mid-Morning limit, unrealized P/L would be ~+$103.32 (+1.9%) by midday. Critical to get this position open.
...
## Pending Actions — OPERATOR MUST EXECUTE (Updated June 8 Midday)

### STEP 0 — IMMEDIATE: CANCEL STALE GTC ORDERS (before placing ANY new orders)
Log into https://app.alpaca.markets → Orders → Open Orders. Cancel ALL of these:
| Order | Why Cancel |
|---|---|
| AMD 9sh GTC limit $524.15 (June 3) | AMD ~$493 — limit above market, was never filled; STALE |
| AMD 9sh GTC limit $520.59 (May 29) | Same — both AMD orders together = potential 18sh guardrail violation |
| PLTR 10sh GTC limit $150.74 (June 3) | PLTR ~$137 — far above market; STALE |
| MRVL 8sh GTC limit $202.19 (May 29) | MRVL ~$311 — far below market; **DO NOT LET THIS FILL** |
| INTC 44sh GTC limit $109.55 (if placed at Mid-Morning) | INTC now $111.90; stale; replace with $112.46 |

### STEP 1 — MANDATORY ORDERS (AFTERNOON ROUTINE — ALL BLOCKED FROM CLOUD)

**OPERATOR must execute at https://app.alpaca.markets**

1. **INTC (MANDATORY — score 8.0, breakout-volume):**
   - BUY **44sh INTC limit $112.46**, stop **$106.84** (-5%), target **$129.33** (+15%)
   - `order_class: bracket, time_in_force: gtc`
   - Size: $4,948 = 4.95% ✓ | Risk: $247 = 0.247% ✓ | R/R: 3.0:1 ✓
   - Catalyst: Google 3M TPU order (Intel 18A) + NVIDIA Feynman GPU evaluation

2. **MU (MANDATORY — score 7.17, mean-reversion-oversold):**
   - BUY **4sh MU limit $940.00**, stop **$893.00** (-5%), target **$1,081.00** (+15%)
   - `order_class: bracket, time_in_force: gtc`
   - Size: $3,760 = 3.76% ✓ | Risk: $188 = 0.188% ✓ | R/R: 3.0:1 ✓
   - **⚠️ MANDATORY EXIT by June 22** (48h before June 24 earnings blackout)
   - NOTE: Today's range was $886–$961; $940 may already be filled — CHECK ALPACA FIRST

3. **MRVL (MANDATORY — score 7.5, new setup, S&P 500 inclusion catalyst):**
   - FIRST: Cancel stale MRVL GTC $202.19 limit (May 29)
   - BUY **8sh MRVL limit** = current ask × 1.005 (~$312.56 est.), stop = fill × 0.95 (~$296.93), target = fill × 1.15 (~$359.44)
   - `order_class: bracket, time_in_force: gtc`
   - Size: ~$2,488 = 2.49% ✓ | Risk: ~$124 = 0.12% ✓ | R/R: 3.0:1 ✓
   - Catalyst: S&P 500 inclusion this month + Jensen Huang "next trillion" + Teralynx T100

4. **GLD stop verification:** Confirm $397.92 GTC stop-sell (7sh) still resting. Do NOT trail.

### STEP 2 — VERIFY (after placing orders)
- Confirm total semis exposure: INTC ($4,948) + MU ($3,760) + MRVL ($2,488) = $11,196 = 11.2% → under 25% ✓
- Confirm cash floor: $100,000 - $2,996 (GLD) - $4,948 - $3,760 - $2,488 = $85,808 = 85.8% → above 5% ✓
- Confirm max 12 positions: 4 total (GLD + INTC + MU + MRVL) → well under ✓

---

## Pending Actions — OPERATOR MUST EXECUTE (PRIOR — June 5 Daily Review — superseded by June 8 Midday above)
