...
## Last Updated
**2026-06-17 (Wednesday) — Market-Open (9:45 AM ET / 13:46 UTC). TRADING DAY — FOMC DECISION DAY. Alpaca API STILL BLOCKED ("Host not in allowlist" — 37th consecutive blocked session). Pre-Market SILENTLY FAILED (violation logged). Market-Open ✓ RUNNING. FOMC decision TODAY 2PM ET (Kevin Warsh first SEP dot plot — KEY RISK). ⚠️⚠️ AMD CRITICAL: AMD sold off from $548.95 (today high) to $507.29 (day low), CROSSING THROUGH both stale GTC buy limits at $524.15 and $520.59 — both may have FILLED without bracket stops. OPERATOR: CHECK ALPACA IMMEDIATELY. If filled: place GTC stop-loss at $481.96 (5% below $507) for each AMD position. 0 confirmed open positions (all stale GTC status unknown). All watchlist entries (MRVL 7.67, INTC 7.17, AMD 7.50, NVDA 7.83) SKIPPED per Exemption 2 (FOMC binary event active through 2PM ET). MARKET: S&P futures +0.3–0.5%; semis rebounding pre-market (INTC, MRVL, MU rallying per web data); AMD ~$507 (off $549 high — FOMC caution). MANDATORY JUNE 18 ENTRIES: MRVL 8sh, INTC 38sh, AMD 9sh (subject to AMD stale GTC resolution), NVDA 4sh (conditional fresh 6-agent). Hawkish dot plot Scenario A: S&P -2%+ → lower entry prices June 18 (buy the dip). Neutral Scenario B: enter at ask×1.005 June 18. JUNETEENTH JUNE 19 = MARKET CLOSED. MU EXIT DEADLINE: JUNE 20 EOD. Cumulative gap: ~−5.07 pp + June 16-17 movement TBD. ⚠️ 20-DAY UNDERPERFORMANCE FLAG: 28 consecutive trading days.**
